...
struct UsbRuntime {
    ...
    uint32_t report_raw_window;
    uint32_t report_unique_window;
    uint32_t report_last_hash;
    bool report_has_hash;
...
uint32_t fnv1a32_update(uint32_t hash, const uint8_t *data, size_t len) {
...
uint32_t hash_nintendo_report_for_dedupe(const uint8_t *report, size_t len) {
    if (!report || len == 0) {
        return 0;
    }
    uint8_t copy[NS2_USB_NINTENDO_REPORT_SIZE];
    const size_t copy_len = len < sizeof(copy) ? len : sizeof(copy);
    memcpy(copy, report, copy_len);
    if (copy_len > 1) {
        copy[1] = 0; // rolling report sequence
    }
    if (copy_len > REPORT_TIMESTAMP_OFFSET + 3) {
        copy[REPORT_TIMESTAMP_OFFSET] = 0;
        copy[REPORT_TIMESTAMP_OFFSET + 1] = 0;
        copy[REPORT_TIMESTAMP_OFFSET + 2] = 0;
        copy[REPORT_TIMESTAMP_OFFSET + 3] = 0;
    }
    return fnv1a32_update(2166136261u, copy, copy_len);
}

void note_report_rate(uint32_t hash) {
    const uint64_t now = time_us_64();
    if (usb.report_window_start_us == 0) {
        usb.report_window_start_us = now;
    }

    usb.report_raw_window++;
    if (!usb.report_has_hash || usb.report_last_hash != hash) {
        usb.report_has_hash = true;
        usb.report_last_hash = hash;
        usb.report_unique_window++;
        usb.report_unique_total++;
    }

    const uint64_t elapsed = now - usb.report_window_start_us;
    if (elapsed >= 1000000u) {
        usb.report_raw_hz = static_cast<uint32_t>((static_cast<uint64_t>(usb.report_raw_window) * 1000000u + elapsed / 2u) / elapsed);
        usb.report_unique_hz = static_cast<uint32_t>((static_cast<uint64_t>(usb.report_unique_window) * 1000000u + elapsed / 2u) / elapsed);
        usb.report_raw_window = 0;
        usb.report_unique_window = 0;
        usb.report_window_start_us = now;
    }
}
...
bool send_report_now() {
    ...
    const uint32_t dedupe_hash = hash_nintendo_report_for_dedupe(report, sizeof(report));
...
void format_usb_status_json(char *out, size_t out_len) {
    ...
    snprintf(out,
             ...
             static_cast<unsigned long>(stats.report_unique_total),
...
void format_usb_config_json(char *out, size_t out_len) {
    snprintf(out,
             ...
             "\"reports_failed\":%lu,\"report_raw_hz\":%lu,"
             "\"report_unique_hz\":%lu,\"report_unique_total\":%lu}",
             ...
             static_cast<unsigned long>(usb.report_raw_hz),
             static_cast<unsigned long>(usb.report_unique_hz),
             static_cast<unsigned long>(usb.report_unique_total));
}
...
#else
        ns2_config_set_display_enabled(false);
#endif
        format_settings_json(json, sizeof(json));
        queue_feature_json(json);
        return;
...
void handle_input_command(const char *command) {
    if (command_is(command, "input recalibrate") ||
        command_is(command, "stick recalibrate") ||
        command_is(command, "axis recalibrate")) {
        ns2_input_reset_axis_calibration();
        queue_feature_json("{\"ok\":true,\"profile\":\"ns2pro\",\"axis_calibration\":\"reset\",\"note\":\"keep_sticks_centered\"}");
        return;
    }

    queue_feature_error("unknown_input_command");
}
...
void handle_feature_command(const char *raw_command) {
    ...
    if (command_has_prefix(command, "input") ||
        command_has_prefix(command, "stick") ||
        command_has_prefix(command, "axis")) {
...
void ns2_usb_get_stats(Ns2UsbStats *out) {
    ...
    out->report_raw_hz = usb.report_raw_hz;
    out->report_unique_hz = usb.report_unique_hz;
    out->report_unique_total = usb.report_unique_total;
...
bool ns2_usb_handle_debug_command(const char *line, char *out, size_t out_len) {
    ...
    if (!command_has_prefix(command, "rumble") &&
        ...
        !command_has_prefix(command, "input") &&
        !command_has_prefix(command, "stick") &&
        !command_has_prefix(command, "axis") &&
    ...
    if (command_has_prefix(command, "rumble")) {
        handle_rumble_command(command);
    } else if (command_has_prefix(command, "input") ||
               command_has_prefix(command, "stick") ||
               command_has_prefix(command, "axis")) {
        handle_input_command(command);
...
uint16_t tud_hid_get_report_cb(uint8_t instance,
                               ...
                               uint8_t *buffer,
                               uint16_t reqlen) {
    return ns2_usb_hid_get_report_cb(instance,
                                     report_id,
...
void tud_hid_set_report_cb(uint8_t instance,
                           ...
                           uint8_t const *buffer,
                           uint16_t bufsize) {
    ns2_usb_hid_set_report_cb(instance,
                              report_id,
                              static_cast<uint8_t>(report_type),
                              buffer,
                              bufsize);
