...
constexpr uint16_t AXIS_PHYSICAL_FULL_SCALE = 1600;
constexpr uint16_t AXIS_STABLE_MAX_DELTA = 16;
...
ns2_input_callback_t input_callback = nullptr;
...
bool axis_delta_within(uint16_t a, uint16_t b, uint16_t limit) {
    const int32_t delta = static_cast<int32_t>(a) - static_cast<int32_t>(b);
    return (delta < 0 ? -delta : delta) <= limit;
}

bool axes_stable(AxisCalibration *axis,
                 uint16_t lx,
                 uint16_t ly,
                 uint16_t rx,
                 uint16_t ry) {
...
void reset_axis_learning(AxisCalibration *axis) {
    axis->sample_count = 0;
    axis->sum_lx = 0;
    axis->sum_ly = 0;
    axis->sum_rx = 0;
...
uint16_t normalize_axis_12bit(uint16_t value, uint16_t center) {
    int32_t delta = static_cast<int32_t>(value) - static_cast<int32_t>(center);
    const bool negative = delta < 0;
    int32_t magnitude = negative ? -delta : delta;
    if (magnitude <= AXIS_DEADZONE || AXIS_PHYSICAL_FULL_SCALE <= AXIS_DEADZONE) {
        return CENTER_12BIT;
    }

    const int32_t usable = AXIS_PHYSICAL_FULL_SCALE - AXIS_DEADZONE;
    const int32_t target = negative ? CENTER_12BIT : (4095 - CENTER_12BIT);
    int32_t scaled = ((magnitude - AXIS_DEADZONE) * target + usable / 2) / usable;
    if (scaled > target) {
        scaled = target;
    }

    return clamp12(negative ?
                   static_cast<int32_t>(CENTER_12BIT) - scaled :
                   static_cast<int32_t>(CENTER_12BIT) + scaled);
}
...
void apply_axes(AxisCalibration *axis,
                ...
                uint16_t rx,
                uint16_t ry) {
    const bool can_learn_center = state->buttons == 0 && axes_stable(axis, lx, ly, rx, ry);
    if (!axis->calibrated && can_learn_center) {
        axis->sum_lx += lx;
        axis->sum_ly += ly;
        axis->sum_rx += rx;
        axis->sum_ry += ry;
    ...
    state->lx = normalize_axis_12bit(lx, axis->center_lx);
    state->ly = normalize_axis_12bit(ly, axis->center_ly);
    state->rx = normalize_axis_12bit(rx, axis->center_rx);
    state->ry = normalize_axis_12bit(ry, axis->center_ry);
}
...
void ns2_input_reset_axis_calibration() {
    reset_axis(&fd2_axis);
    reset_axis(&legacy_axis);
    input.lx = CENTER_12BIT;
    input.ly = CENTER_12BIT;
    input.rx = CENTER_12BIT;
    input.ry = CENTER_12BIT;
    printf("[NS2 INPUT] axis calibration reset\n");
}

void ns2_input_register_callback(ns2_input_callback_t callback) {
