...
void print_help() {
    printf("commands: status | usb status | usb rate <hz> | usb raw on|off | motion status | stick recalibrate | rumble config|tune|test|stop | settings status|save | display on|off | web parse on|off | ns2 scan | ns2 pair | ns2 reconnect | ns2 disconnect | ns2 forget | ns2 auto on|off | ns2 candidates | bootrom\n");
}
